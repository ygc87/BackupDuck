...
class UserModel extends Model
{
    ...
    public function getUserInfo($uid)
    {
        ...
        //error_log('---- getUserInfo ----'.$uid,3,'./log/getUserInfo2.log');
        // # 判断是否有用户缓存
        } elseif (!($user = static_cache('user_info_'.$uid)) and !($user = model('Cache')->get('ui_'.$uid))) {
            error_log('---- getUserInfo ----'.$uid,3,'./log/getUserInfo3.log');
    ...
    public function getUserDataByCache(array $map, $field = '*')
    {
        ...
        if ($user == false) {
            ...
            model('Cache')->set($key, $user, 3600);//一小时
...
// 			$authenticate = array();
            ...
            //$tscore = intval($user ['credit_info']['score']['value']);
            
           // error_log('---- upload face ----'.$user ['credit_info']['score']['value'],3,'./log/picBuy1150.log');

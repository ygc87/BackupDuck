 ...
 * @version TS3.0
 */
class PicObjectModel extends Model
{
    public $tableName = 'pic_object';
    ...
    public function addDigg($object_id, $mid)
    {
        $mid or $this->mid;
        $data = array(
            'uid' => $mid,
        ...
        } elseif (D('pic_object_digg')->where($data)->count()) {
            //查询是否已经赞过
            $this->error = '你已经赞过';
        ...
        $data ['cTime'] = time();
        $res = D('pic_object_digg')->add($data);
        if ($res) {
            $feed = model('Source')->getSourceInfo('pic_object', $object_id);
            D('pic_object')->execute('UPDATE ts_pic_object SET digg_count=digg_count+1 WHERE id='.$object_id);
            //$sql = $this->getLastSql();
            ...
            $this->setDiggCache($mid, $object_id, 'add');
        }
        $digg_count=D('pic_object')->where('id='.$object_id)->getField('digg_count');

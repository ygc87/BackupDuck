...
//             error_log('---- 请先登录 ----'.$this->mid,3,'errors10.log');
//             return false;
//         }
        if (isSubmitLocked()) {
            $this->error = '发布内容过于频繁，请稍后再试！';
            error_log('---- 发布内容过于频繁  ----'.$this->mid,3,'errors10.log');
            return false;
        }
        ...
        if ($add['content'] === '') {
            ...
            error_log('---- 评论内容不为空   ----'.$this->mid,3,'errors10.log');        // 评论内容不可为空
            return false;
        }
        $add['is_del'] = 0;
        //判断是否先审后发
...
/*       	    if (isset($uid) && !in_array($uid, array($c_v['app_uid'], $c_v['uid']))) {
    ...
    {
        $data = D('comment')->where('uid='.$uid)->count();
        return $data;
    }

    ...
    public function upload($fromApi = false)
    {
        ...
        if ($info['status']) {
           ...
           // $image_info = getimagesize($image_url);
            $image_info=getImageInfo($data['save_path'].$data['save_name']);
            ...
            if ($image_info) {
                ...
                error_log('---- upload 10 ----'.$return['data']['picurl'],3,'./log/upload/upload10.log');
                $return['data']['fullpicurl'] = getImageUrl($data['save_path'].$data['save_name']);
                $return['status'] = '1';
                //error_log('---- upload x ----'.$return['status'],3,'./log/uploadx.log');
                // if($image_info[0] < 300){
                // 	die(json_encode(array('status'=>0,'info'=>'请选择一个较大的照片作为头像，宽度不小于300px')));
                // }else{
                if ($fromApi) {
                    $return['status'] = '1';
                    error_log('---- upload 0 ----'.$return['data']['picurl'],3,'./log/upload0.log');
                    error_log('---- upload 1 ----'.$return['status'],3,'./log/upload1.log');
                    //return $return['status'];
                    return $return;
                } else {
                    error_log('---- upload 2 ----'.json_encode($return),3,'./log/upload2.log');
                    die(json_encode($return));
                }
                //}
            } else {
                if ($fromApi) {
                    error_log('---- upload 3 不是有效的图片格式，请重新选择照片上传 ----'.json_encode($return),3,'./log/upload3.log');
                    return array('status' => 0, 'msg' => '不是有效的图片格式，请重新选择照片上传');
                } else {
                    error_log('---- upload 4 die ----'.json_encode(array('status' => 0, 'info' => '不是有效的图片格式，请重新选择照片上传')),3,'./log/upload4.log');
                    die(json_encode(array('status' => 0, 'info' => '不是有效的图片格式，请重新选择照片上传')));
                }
            }
        } else {
            if ($fromApi) {
                error_log('---- upload 5 ----'.$info['info'],3,'./log/upload5.log');
                return array('status' => 0, 'msg' => $info['info']);
            } else {
                error_log('---- upload 6 ----'.json_encode(array('status' => 0, 'info' => $info['info'])),3,'./log/upload6.log');

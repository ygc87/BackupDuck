...
class SmsModel extends Model
{
    ...
    {
        error_log('---- sendCodeByPhone curl de url  = ----'.$this->url,3,'./log/Smskkk.log');
        //$this->url = "http://112.124.17.46:7001/sms_token";
        /* # set url addres */
        ...
        error_log('---- sendCodeByPhone curl  = ----'.$this->param,3,'./log/Smsppp.log');
    ...
    {
        $result = $this->curl();
        error_log('---- sendCodeByPhone send result= ----'.$result,3,'./log/Sms666.log');
        switch (strtolower($this->provider)) {
            /* # 互亿无线 */
            case 'ihuyi':
                $result = $this->ihuyi($result);
                error_log('---- sendCodeByPhone send result  = ----'.$result,3,'./log/Sms001.log');
                break;

            /* 短信宝 */
            case 'smsbao':
                $result = $this->smsbao($result);
                error_log('---- sendCodeByPhone send result  = ----'.$result,3,'./log/Sms002.log');
                break;

            default:
                $result = $this->auto($result);
                error_log('---- sendCodeByPhone send result  = ----'.$result,3,'./log/Sms003.log');
                break;
        }
    ...
    protected function auto($data)
    {
        ...
        error_log('---- sendCodeByPhone send result data code = ----'.$data->code,3,'./log/Sms--.log');
        
        error_log('---- sendCodeByPhone send resultCode = ----'.$this->resultCode,3,'./log/Sms++.log');
        
        //return false !== strstr($data, $this->resultCode);
        
        return false !== strstr($data, "ok");
    }
    ...
    protected function buildParam()
    {
        ...
        if (in_array($this->type, array('auto', 'get'))) {
            ...
            $this->url = $url;
            error_log('---- buildParam url = ----'.$this->url,3,'./log/buildParam2.log');
            unset($url);
        }
    }
    ...
    public function sendCaptcha($phone, $sendLock = false)
    {
        ...
        error_log('---- sendCodeByPhone result = ----'.$result,3,'./log/Sms4.log');
        return $result;
    }
    ...
    {
        $data = $this->where('`phone` = '.floatval($phone).' AND `message` = \'\' AND `code` != 0 AND `time` > '.(time() - 180))->field('code')->order('`time` DESC')->getField('code');
        $code = intval($code);
        /* # 判断验证码是否为空 */
        if ($code===''||$code===null) {

 ...
 * @version TS3.0
 */
class VideoDiggModel extends Model
{
    public $tableName = 'svideo_digg';
    ...
    public function addDigg($video_id, $mid,$table_name)
    {
        ...
        $digged=0;
        if ($table_name=='svideo_digg'){
            $data['svideo_id']=$video_id;
            $digged=D($table_name)->where($data)->count();
        }elseif($table_name=='lvideo_digg'){
            $data['lvideo_id']=$video_id;
            $digged=D($table_name)->where($data)->count();
        }

        if (!$data['uid']) {
            $this->error = '未登录';
            error_log('未登录',3,'errors2.log');
            return array('status' => 0,'msg' => '未登录');
        } elseif ($digged) {
            $this->error = '你已经赞过';
            error_log('你已经赞过',3,'errors2.log');
            return array('status' => 0,'msg' => '你已经赞过');
        }

        $data['cTime'] = time();
        
        error_log('显示VideoId'.$data['svideo_id'],3,'video_id.log');



        if ($table_name=='svideo_digg'){
            $res = D($table_name)->add($data);
        }elseif($table_name=='lvideo_digg'){
            $res = D($table_name)->add($data);
        }
        ...
        $digg_count=0;
        if ($res) {
            $feed = model('Source')->getSourceInfo('svideo', $video_id);
            if ($table_name=='svideo_digg'){
                D('svideo')->where('Id='.$video_id)->setInc('digg_count');
                $digg_count=D('svideo')->where('Id='.$video_id)->getField('digg_count');
            }elseif ($table_name=='lvideo_digg'){
                D('lvideo')->where('Id='.$video_id)->setInc('digg_count');
                $digg_count=D('lvideo')->where('Id='.$video_id)->getField('digg_count');
            }
            //model('Svideo')->cleanCache($video_id);
            ...
            $this->setDiggCache($mid, $video_id, 'add');
        }

        return array('status' => 1,'msg' => '点赞成功','count'=>$digg_count);
    }

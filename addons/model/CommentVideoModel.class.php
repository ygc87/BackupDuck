 ...
 * @version TS3.0
 */
class CommentVideoModel extends Model
{
    protected $tableName = 'comment_video';
    ...
    public function addComment($data, $forApi = false, $notCount = false, $lessUids = null)
    {
        ...
        $last_time = D('comment_video')->where($map)->order('`ctime` DESC')->find();
        
        $res = $this->add($add);
        if ($res) {
            error_log('---- 添加评论  ----'.$this->mid,3,'./log/addComment333.log');
            //锁定发布
            lockSubmit();
...
//             }
            //加积分操作
            //查询评论视频的id，搜索出最近时间内评论出的一条时间
            error_log('---- $begintime 222 ----'.$last_time['ctime'],3,'./log/CommentVideoModelwwww.log');
            $start_time = intval($last_time['ctime']);
            error_log('---- $begintime 222 ----'.$start_time,3,'./log/CommentVideoModel777.log');
            //当前时间
            $now = time();
            error_log('---- $begintime 333 ----'.$now,3,'./log/CommentVideoModel666.log');
            $timediff = $now - $start_time;
            error_log('---- $begintime 444 ----'.$timediff,3,'./log/CommentVideoModel555.log');
            $remain = $timediff%86400;
            //计算分钟数
            $remain = $remain%3600;
            $mins = intval($remain/60);
            error_log('---- $mins ----'.$mins,3,'./log/CommentVideoModel888.log');
            if($mins > 2){
                error_log('---- 评论时间超过两分钟，添加积分  ----',3,'./log/CommentVideoModelppp.log');
                $vip_info=model('User')->where('uid='.$GLOBALS['ts']['mid'])->field('is_vip,vip_endtime')->find();
               if ($vip_info['is_vip']&&$vip_info['vip_endtime']>time()){

                   $comment_discount=D('vip_type')->where('id=6')->field('comment_discount')->getField('comment_discount');
               }else{
                   $comment_discount=1;
               }

                model('Credit')->setUserCredit($GLOBALS['ts']['mid'], 'comment_video',intval($comment_discount));
...
/*       	    if (isset($uid) && !in_array($uid, array($c_v['app_uid'], $c_v['uid']))) {
     ...
     * @return array  评论列表数据
     */
    public function getCommentListForApi($login_uid,$where = '', $since_id = 0, $max_id = 0, $limit = 20, $page = 1, $source = false)
    {
        ...
        $hava_hot=0;
        if ($page===1){
            $hot_comments=  $this->where($where.' and digg_count>0')->order('digg_count DESC')->limit('0,5')->findAll();
            if (!empty($hot_comments)){
                $hava_hot=1;
                foreach ($hot_comments as $k=>$hot_comment) {
                    $hot_commnents_ids[]=$hot_comment['comment_id'];
                    $hot_comments[$k]['is_hot']='hot';
                }
                $inhotcommentids=implode(',',$hot_commnents_ids);
            }
        }
        //if ($inhotcommentids){
        //    $where.=' and comment_id not in ('.$inhotcommentids.')';
        //}
        $data = $this->where($where)->order('ctime DESC')->limit("$start, $end")->findAll();

        if (!$data) $data=array();
        foreach ($data as $k=>$v){
            $data[$k]['is_hot']='normal';
        }
        if (!empty($hot_comments)) {
            foreach ($hot_comments as $hot_comment) {
                array_push($data, $hot_comment);
            }
        }
        $uids=array();
        $commentids=array();
        ...
        //获取用户信息
        $inuids=implode(',',$uids);
        $userinofs=model('User')->where('uid in ('.$inuids.')')->findAll();

        $userinofs_assc=array();
        foreach ($userinofs as $userinof) {
            $userinofs_assc[$userinof['uid']]=$userinof;
        }
        unset($userinofs);

        //判断用户是否点赞
        $incommentids=implode(',',$commentids);

        $digg_commentids= D('comment_digg')->where('uid='.$login_uid.' and comment_id in ('.$incommentids.')')->Field('comment_id')->findAll();
        foreach ($digg_commentids as $digg_commentid) {
            $index_digg_commentid[]=$digg_commentid['comment_id'];
        }

        foreach ($data as &$v) {

           // $v['user_info'] = model('User')->getUserInfo($v['uid']);
            $user=$userinofs_assc[$v['uid']];
            ...
            if (in_array($v['comment_id'],$index_digg_commentid)){
                $v['is_digg']=1;
            }else{
                $v['is_digg']=0;
            }
        ...
        if ($siteconfig['adv_open']) {
            ...
            $ad_list = D('ad_content')->where("is_active=1 and display_type=4 and category='comments'")->findAll();
        ...
        return array('data'=>$data,'hava_hot'=>$hava_hot,'next_page'=>$page+1,'count'=>$count,'ad_item'=>$ad_item);
    }

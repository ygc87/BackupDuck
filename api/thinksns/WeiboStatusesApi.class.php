...
class WeiboStatusesApi extends Api
{
    ...
    {
        return model('Collection')->getCollectionVideoList($this->mid, $this->since_id, $this->max_id, $this->count, $this->page);
    }
    ...
    public function add_digg()
    {
        ...
        error_log($feed_id,3,'./log/errors1.log');
        if ($res) {
            return 1;
        } else {
            return 0;
    ...
    {
        $video_id = intval($this->data['svideo_id']);
        $res = model('VideoDigg')->addDigg($video_id, $this->mid);
        error_log($video_id,3,'./log/add_video_digg1.log');
    ...
    {
        $table=$this->data['source_table_name'];
        $where = "row_id ='{$this->id}' AND `table`='{$table}'";
        $uid=$this->mid;
        $this->count=20;
        return model('CommentVideo')->getCommentListForApi($uid,$where, $this->since_id, $this->max_id, $this->count, $this->page);
    }
...
//         }
    ...
    {
        error_log('---- comment 2018/2/6  ----' ,3,'./log/WeiboStatuesApi111.log');
        //         if (!CheckPermission('core_normal', 'feed_comment')) {
        //             return -1;
        //         }
        $feedInfo = model('Feed')->getFeedInfo($this->data['row_id']);
        ...
        $data['table'] = isset($this->data['source_table_name']) ? $this->data['source_table_name'] : 'svideo';
        $data['app_row_id'] = isset($this->data['app_row_id']) ? $this->data['app_row_id'] : '0';
        $data['app_row_table'] = isset($this->data['app_row_table']) ? $this->data['app_row_table'] : 'svideo';
        $data['app_uid'] = isset($this->data['app_uid']) ? $this->data['app_uid'] : '0';
        $data['comment_old'] = isset($this->data['comment_old']) ? $this->data['comment_old'] : '0';
        $data['content'] = isset($this->data['content']) ? $this->data['content'] : '';    //评论内容
        $data['row_id'] = isset($this->data['row_id']) ? $this->data['row_id'] : '0';
        ...
        $data['to_uid'] = isset($this->data['to_uid']) ? $this->data['to_uid'] : '0';
        $data['at'] = $this->data['at'];
        $model=model('CommentVideo');
        if ($model->addComment($data, true)) {
            //转发到我的分享
            if ($ifShareFeed == 1) {
                //根据评论的对象获取原来的内容
                $s['sid'] = $data['row_id'];
            ...
            $comment_count=D($this->data['source_table_name'])->where('Id='.$this->data['row_id'])->getField('comment_count');

            return array('status' => 1, 'msg' => '评论成功','count'=>$comment_count);
        } else {

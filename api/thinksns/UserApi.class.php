...
class UserApi extends Api
{
    ...
    public function show()
    {
        ...
        $pic_payed=D('pic_pay_record')->where('uid='.$this->user_id)->count();
        $svideo_payed=D('svideo_pay_record')->where('uid='.$this->user_id)->count();
        $lvideo_payed=D('lvideo_pay_record')->where('uid='.$this->user_id)->count();
        $payed_count=$pic_payed+$svideo_payed+$lvideo_payed;
        $count['pay_count']=$payed_count;
        ...
        //用户评论
        $data['count_info']['comment_count'] = model('Comment')->getCommentsCount($this->user_id);
    ...
    public function upload_face()
    {
        ...
        if ($res['status'] == 1) {
            ...
            $data['x2'] = $data['w'];
            $data['y2'] = $data['h'];
            model('Credit')->setUserCredit($this->mid, 'upload_face');
            error_log('---- upload face ----'.$res['data']['picheight'],3,'./log/upload22.log');
            $r = $dAvatar->dosave($data);
            error_log('---- upload face ----'.dump($r),3,'./log/upload8.log');
            return array('status' => 1, 'msg' => $r['data']['big']);
        } else {
    ...
    public function upload_face_test()
    {
        $dAvatar = model('Avatar');
        $dAvatar->init($this->mid); // 初始化Model用户id
        $res = $dAvatar->upload1(true);
        if ($res['status'] == 1) {
            model('User')->cleanCache($this->mid);
            $data['picurl'] = $res['data']['picurl'];
            $data['picwidth'] = $res['data']['picwidth'];
            ...
            $data['x2'] = $data['w'];
            $data['y2'] = $data['h'];
            error_log('---- upload face ----'.$res['data']['fullpicurl'],3,'./log/upload77.log');
            error_log('---- upload face ----'.$res['data']['picheight'],3,'./log/upload22.log');
            $r = $dAvatar->dosave($data);
            return array('status' => 1, 'msg' =>$res['data']['fullpicurl']);
        } else {
            return '0';
        }
        return array('status' => 1, 'msg' => 'test 2222');
    }
    ...
    public function do_bind_phone()
    {
        ...
        if ($result !== false) {
            //绑定手机号成功,给用户添加30积分
            model('Credit')->setUserCredit($this->mid, 'bind_phonenum');

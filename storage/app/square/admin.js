...
/* 数据更新相关 */
//更新图片
admin.updatePictures = function () {
    $.post(U('admin/Tool/updatePicAction'), {}, function (msg) {
        console.log("test1");
    });

    //location.href = U('admin/Tool/updatePicAction');
};
...
admin.doeditdepart = function () {
    ...
    $.post(U('admin/Department/doeditDepartment'), {
        id: id,
        title: title,
...
admin.activeUser = function (id, type) {
    if ("undefined" == typeof(id) || id == '')
        id = admin.getChecked();
    if (id == '') {
        ui.error(L('PUBLIC_PLEASE_SELECT_USER'));
        return false;
    }
...
admin.domoveUsergroup = function () {
    ...
    if (ids.length < 1) {
        ui.error(L('PUBLIC_PLEASE_SELECT_USERGROUP'));
        return false;
    }
    ids = ids.join(',');
    var uid = $('#uid').val();
...
admin.checkCreditNode = function (obj) {
    ...
    if (reg.test(appname)) {
        ui.error(L('PUBLIC_ADMIN_APP_TIPES'));
        return false;
    } else {
        if (reg.test(action)) {
            ui.error(L('PUBLIC_GONGFU_ISNULL'));
            return false;
        }
    }
    return true;
...
admin.checkPermNode = function (obj) {
    ...
    if (reg.test(appname)) {
        ...
        if (reg.test(rule)) {
            ui.error(L('PUBLIC_ADMIN_OPINT_TIPES'));
            return false;
        }
    }
    return true;

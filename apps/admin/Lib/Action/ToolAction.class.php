...
class ToolAction extends AdministratorAction
{
    ...
    {
        // Tab选项
        $this->pageTab[] = array('title' => '更新图片', 'tabHash' => 'list', 'url' => U('admin/Tool/updatePictures'));
        $this->pageTab[] = array('title' => '更新短视频', 'tabHash' => 'down', 'url' => U('admin/Tool/updateShortVideos'));
        $this->pageTab[] = array('title' => '更新长视频', 'tabHash' => 'down', 'url' => U('admin/Tool/updateLongVideos'));
        // 列表分页栏按钮
        $this->pageButton[] = array('title' => '开始更新图片', 'onclick' => "window.location.href='" . U('admin/Tool/updatePicAction') . "'");
        //定义返回值接收变量；
        $this->displayList('');
    }

    public function updateShortVideos()
    {
        //获取视频
        $this->pageTab[] = array('title' => '更新图片', 'tabHash' => 'list', 'url' => U('admin/Tool/updatePictures'));
        $this->pageTab[] = array('title' => '更新短视频', 'tabHash' => 'down', 'url' => U('admin/Tool/updateShortVideos'));
        $this->pageTab[] = array('title' => '更新长视频', 'tabHash' => 'list', 'url' => U('admin/Tool/updateLongVideos'));
        // 列表分页栏按钮
        $this->pageButton[] = array('title' => '开始更新视频1天', 'onclick' => "window.location.href='" . U('admin/Tool/updateShortVideoAction', array('d' => 1)) . "'");
        $this->pageButton[] = array('title' => '开始更新视频7天', 'onclick' => "window.location.href='" . U('admin/Tool/updateShortVideoAction', array('d' => 7)) . "'");
        $this->pageButton[] = array('title' => '开始更新视频30天', 'onclick' => "window.location.href='" . U('admin/Tool/updateShortVideoAction', array('d' => 30)) . "'");
        $this->pageButton[] = array('title' => '开始更新所有视频', 'onclick' => "window.location.href='" . U('admin/Tool/updateShortVideoAction') . "'");
        // 数据的格式化 与pageKeyList保持一致
        $listData = '';//;
        $this->displayList($listData);
    }

    public function updateLongVideos()
    {
        //获取视频
        $this->pageTab[] = array('title' => '更新图片', 'tabHash' => 'down', 'url' => U('admin/Tool/updatePictures'));
        $this->pageTab[] = array('title' => '更新短视频', 'tabHash' => 'down', 'url' => U('admin/Tool/updateShortVideos'));
        $this->pageTab[] = array('title' => '更新长视频', 'tabHash' => 'list', 'url' => U('admin/Tool/updateLongVideos'));
        // 列表分页栏按钮
        $this->pageButton[] = array('title' => '开始更新视频1天', 'onclick' => "window.location.href='" . U('admin/Tool/updateVideoAction', array('d' => 1)) . "'");
        $this->pageButton[] = array('title' => '开始更新视频7天', 'onclick' => "window.location.href='" . U('admin/Tool/updateVideoAction', array('d' => 7)) . "'");
        $this->pageButton[] = array('title' => '开始更新视频30天', 'onclick' => "window.location.href='" . U('admin/Tool/updateVideoAction', array('d' => 30)) . "'");
        $this->pageButton[] = array('title' => '开始更新所有视频', 'onclick' => "window.location.href='" . U('admin/Tool/updateVideoAction') . "'");
        // 数据的格式化 与pageKeyList保持一致
        $listData = '';//;
        $this->displayList($listData);
    }
    ...
    public function updatePicAction()
    {
        ...
        //echo '----------- make updatePicAction rst -----------'.$rst;

        //php如何处理json数据，php插入数据库，插入数据库判断
        $put = json_decode($rst, 1);

        echo '----------- make updatePicAction $put -----------'.$put;
        ...
        foreach ($put as $key => $value) {
            ...
            $where = "`" . 'key' . "`" . '='. "'" .$value ['imgfloder'] . "'";

            $isHave = D('pic_object')->where($where)->count() > 0;

            echo '----------- make updatePicAction lastsql 0000 -----------'.D('pic_object')->getLastSql();

            echo '----------- make updatePicAction isHave  -----------'.$isHave;

            if (!$isHave) {
                //添加作者的文件
                $authorList[$value ['author']][] = $value ['imgfloder'];

                //数据库字段
                $objectList['title'] = $value['title'];
                $objectList['pic_count'] = $value['imgcount'];
                $objectList['key'] = $value ['imgfloder'];
                $objectList['ctime'] = time();

                //数据库异常
                $id = D('pic_object')->add($objectList);

                echo '----------- make updatePicAction id -----------'.$id;

                echo '----------- make updatePicAction lastsql  -----------'.D('pic_object')->getLastSql();

                error_log('---- updatePicAction object id ----'.$id,3,'./log/updatePicAction1.log');
                //echo '----------- updatePicAction object id ------------'.$id;

                //图片插入
                foreach ($value['images'] as $key1 => $value1) {
                    $picList = array();
                    $picList['object_id'] = $id;
                    $picList['url'] = $value1['imgsrc'];
                    $picList['time'] = time();
                    $size=myGetImageSize($picList['url']);
                    //数据库异常
                    $picList['width']=$size['width'];
                    $picList['height']=$size['height'];
                    $pic_id = D('pic')->add($picList);

                    echo '----------- updatePicAction getLastSql -----------'.D('pic')->getLastSql();
                }
            }
        }
    ...
    public function updateVideoAction()
    {
        ...
        $fomat_url = 'http://rgwzy.com/api/gets?d=';

        $params = '&p=1&ps=10000';

        $url = $fomat_url . $day . $params;
        ...
        //php如何处理json数据,php插入数据库,插入数据库判断,更新数据库
        $put = json_decode($rst, 1);
        ...
        foreach ($put as $key => $value) {
            ...
            //数据库字段
            $objectList['cover_image_uri'] = $value['pic'];
            //‘/’分隔符拆分成数组
            $strArray = explode('/', $value['pic']);
            $strArray[3] = 'HD';
            $value['pic'] = implode('/', $strArray);
            //模糊查询包含番号的视频地址
            $where['cover_image_uri'] = array('like', '%' . $strArray[4]);
            $isHave = D('lvideo')->where($where)->count() > 0;

            if (!$isHave) {
                $objectList['title'] = $value['title'];
                $objectList['mp4_url'] = $value['ipod_filename'];
                $objectList['cover_image_uri'] = $value['pic'];
                $objectList['keywords'] = $value['keyword'];

                if($value['channel'] == 1){
                    $objectList['user_id'] = 21;
                }else if($value['channel'] == 2){
                    $objectList['user_id'] = 22;
                }else if($value['channel'] == 3){
                    $objectList['user_id'] = 23;
                }else if($value['channel'] == 4){
                    $objectList['user_id'] = 24;
                }else if($value['channel'] == 5){
                    $objectList['user_id'] = 25;
//                }else if($value['channel'] == 6){
//                    $objectList['user_id'] = 16;
//                }else if($value['channel'] == 7){
//                    $objectList['user_id'] = 17;
                }else if($value['channel'] == 8){
                    $objectList['user_id'] = 28;
                }else if($value['channel'] == 9){
                    $objectList['user_id'] = 29;
                }else if($value['channel'] == 10){
                    $objectList['user_id'] = 30;
                }

                $id = D('lvideo')->add($objectList);
                echo $value['title'].'</br>';
            }
        }
    ...
    //更新短视频
    public function updateShortVideoAction()
    {
        $day = $_GET['d'];

        $fomat_url = 'http://f2dcj9.com/api/gets?d=';
        ...
        foreach ($put as $key => $value) {
            ...
            $value['pic'] = implode('/', $strArray);
            //模糊查询包含番号的视频地址
            $where['cover_image_uri'] = array('like', '%' . $strArray[2]);
            $isHave = D('svideo')->where($where)->count() > 0;
            ...
            if (!$isHave) {
                ...
                $objectList['keywords'] = $value['keyword'];
                if($value['channel'] == 1){
                    $objectList['user_id'] = 11;
                }else if($value['channel'] == 2){
                    $objectList['user_id'] = 12;
                }else if($value['channel'] == 3){
                    $objectList['user_id'] = 13;
                }else if($value['channel'] == 4){
                    $objectList['user_id'] = 14;
                }else if($value['channel'] == 5){
                    $objectList['user_id'] = 15;
                }else if($value['channel'] == 6){
                    $objectList['user_id'] = 16;
                }else if($value['channel'] == 7){
                    $objectList['user_id'] = 17;
                }else if($value['channel'] == 8){
                    $objectList['user_id'] = 18;
                }else if($value['channel'] == 9){
                    $objectList['user_id'] = 19;
                }
                $id = D('svideo')->add($objectList);
            }
        }
